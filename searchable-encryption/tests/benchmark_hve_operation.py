...
from .context import hve, util, parse_params_from_string, \
    pairingcurves, pairinggroup, hveutil
...
def benchmark_hve_sigle_set(
    ...
    width = hveutil.check_size(indices, queries)
...
def benchmark_hve_sigle_operation():
    ...
    group_params[384] = parse_params_from_string(pairingcurves.PAIRING_CURVE_TYPE_A1_384_SAMPLE)
    group_params[512] = parse_params_from_string(pairingcurves.PAIRING_CURVE_TYPE_A1_512_SAMPLE)
    group_params[768] = parse_params_from_string(pairingcurves.PAIRING_CURVE_TYPE_A1_768_SAMPLE)
    ...
    for bitLen, groupParam in group_params.items():
        ...
        p = int(groupParam[hveutil.PARAM_KEY_N0])
        q = int(groupParam[hveutil.PARAM_KEY_N1])
        ...
        with open('benchmark_operation.csv', 'a+') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([
                bitLen,
                time_random_element,
